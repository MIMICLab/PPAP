...
with graph.as_default():
    ...
    with sess.as_default():
        ...
        re_true, D_real_logits  = discriminator(input_shape, n_filters, filter_sizes, A_true_flat, var_D)
        re_fake, D_fake_logits  = discriminator(input_shape, n_filters, filter_sizes, G_sample, var_D, reuse = True)
        ...
        G_fake_loss = tf.reduce_mean(tf.nn.sigmoid_cross_entropy_with_logits(logits=D_fake_logits, labels=tf.ones_like(D_fake_logits)))    
        
        D_loss = D_real_loss + D_fake_loss + 0.1*A_D_true_loss + 0.1*A_D_fake_loss
        
        G_loss = G_fake_loss + 0.1*A_G_loss - 0.1*A_D_fake_loss
        ...
        tf.summary.scalar('G_loss',G_fake_loss)   
