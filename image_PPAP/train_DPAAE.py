...
with graph.as_default():
    ...
    with sess.as_default():
        ...
        G_sample,A_sample, z_original,z_dp, lambda_layer,  z_noise = dp_autoencoder(input_shape, n_filters, filter_sizes,z_dim, A_true_flat, Z_noise, var_G)
        ...
        tf.summary.histogram('z_noise_applied',z_dp)         
        ...
        if not os.path.exists('results/DPAAE/'):
            os.makedirs('results/DPAAE/')        
        out_dir = os.path.abspath(os.path.join(os.path.curdir, "results/DPAAE/models/{}_".format(dataset) + timestamp))
        checkpoint_dir = os.path.abspath(os.path.join(out_dir, "checkpoints"))
        checkpoint_prefix = os.path.join(checkpoint_dir, "model")
        if not os.path.exists('results/DPAAE/models/'):
            os.makedirs('results/DPAAE/models/')
        if not os.path.exists(checkpoint_dir):
            os.makedirs(checkpoint_dir)
            saver = tf.train.Saver(tf.global_variables())
        if not os.path.exists('results/DPAAE/dc_out_{}/'.format(dataset)):
            os.makedirs('results/DPAAE/dc_out_{}/'.format(dataset))
        if not os.path.exists('results/DPAAE/generated_{}/'.format(dataset)):
            os.makedirs('results/DPAAE/generated_{}/'.format(dataset))            

        train_writer = tf.summary.FileWriter('results/graphs/DPAAE/{}'.format(dataset),sess.graph)
        ...
        for it in range(1000000000):
            ...
            if it % 1000 == 0: 
                ...
                plt.savefig('results/DPAAE/dc_out_{}/{}.png'.format(dataset,str(i).zfill(3)), bbox_inches='tight')
                plt.close(fig)
                i += 1
            ...
            if it% 100000 == 0 and it!=0:
                ...
                np.save('results/DPAAE/generated_{}/generated_image.npy'.format(dataset), generated)
